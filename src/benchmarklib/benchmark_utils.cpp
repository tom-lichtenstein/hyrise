...
BenchmarkConfig::BenchmarkConfig(const BenchmarkMode benchmark_mode, const bool verbose, const ChunkOffset chunk_size,
    ...
    : benchmark_mode(benchmark_mode),
      ...
      min_num_query_runs(min_num_query_runs),
...
BenchmarkConfig CLIConfigParser::parse_basic_options_json_config(const nlohmann::json& json_config) {
  ...
  const auto min_runs = json_config.value("min_runs", default_config.min_num_query_runs);
  out << "- Min runs per query is " << min_runs << std::endl;
  ...
  return BenchmarkConfig{benchmark_mode,  verbose,  chunk_size,           *encoding_config,
                         min_runs,        max_runs, minimum_duration,     timeout_duration,
                         warmup_duration, use_mvcc, output_file_path,     enable_scheduler,
                         cores,           clients,  enable_visualization, out};
}
...
nlohmann::json CLIConfigParser::basic_cli_options_to_json(const cxxopts::ParseResult& parse_result) {
  ...
  json_config.emplace("min_runs", parse_result["min_runs"].as<size_t>());
  ...
  json_config.emplace("min_time", parse_result["min_time"].as<size_t>());
